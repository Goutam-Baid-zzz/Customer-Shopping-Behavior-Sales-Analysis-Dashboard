...
def load_main_data():
    ...
    # Normalize category-like text columns to avoid duplicate labels
    if 'Category' in df.columns:
        # ensure string, strip extra whitespace, collapse internal spaces,
        # normalize unicode, and unify capitalization
...
def page_home(df, analysis_data):
    ...
    with tab2:
        ...
        metrics_data = {
            ...
            "Customer Lifetime Value": fmt_usd(df['Purchase Amount (USD)'].sum() / df['Customer ID'].nunique()),
    ...
    with tab3:
        ...
        insight("Revenue Risk",
                ...
                f"<strong>{df.groupby('Category')['Purchase Amount (USD)'].sum().min():,.0f} USD</strong> "
...
def page_dataset_preview(df):
    ...
    with tab2:
        ...
        st.markdown("""
        <div style="background:var(--ink-mid);border:1px solid var(--wire);border-top:2px solid var(--blue);
                    border-radius:var(--radius-lg);padding:1rem 1.6rem 0.2rem;margin-bottom:0.8rem;">
            <div style="font-size:0.65rem;font-weight:700;letter-spacing:0.16em;text-transform:uppercase;
                        color:var(--blue);margin-bottom:0.6rem;display:flex;align-items:center;gap:8px;">
                <span style="display:inline-block;width:6px;height:6px;background:var(--blue);
                             border-radius:50%;box-shadow:0 0 6px var(--blue);"></span>
                Filter Controls
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with f1:
            sel_cat = st.multiselect("Category", options=list(df['Category'].unique()),
                                     default=list(df['Category'].unique()[:3]),
                                     label_visibility="visible")
        with f2:
            sel_gen = st.multiselect("Gender", options=list(df['Gender'].unique()),
                                     default=list(df['Gender'].unique()),
                                     label_visibility="visible")
        with f3:
            min_rat = st.slider("Min Rating ★", 1.0, 5.0, 1.0, step=0.5)

        fdf = df[df['Category'].isin(sel_cat) & df['Gender'].isin(sel_gen)
...
def page_data_quality(df, analysis_data, images):
    ...
    with tab1:
        ...
        if 'missingness_heatmap' in images:
            ...
            st.image(images['missingness_heatmap'], use_container_width=True)
    ...
    with tab1:
        ...
        with c1:
            if '01_customer_purchase_distribution' in images:
                st.image(images['01_customer_purchase_distribution'], use_container_width=True)
            else:
    ...
    with tab2:
        ...
        with c1:
            if '02_age_demographics' in images:
                st.image(images['02_age_demographics'], use_container_width=True)
            else:
    ...
    with tab3:
        ...
        with c1:
            if '03_category_sales_volume' in images:
                st.image(images['03_category_sales_volume'], use_container_width=True)
            else:
                cat_d = df['Category'].value_counts()
        ...
        with c2:
            if '05_shipping_preferences' in images:
                st.image(images['05_shipping_preferences'], use_container_width=True)
            else:
                ship_d = df['Shipping Type'].value_counts()
    ...
    with tab4:
        ...
        with c1:
            if '06_satisfaction_analysis' in images:
                st.image(images['06_satisfaction_analysis'], use_container_width=True)
            else:
...
def page_multivariate_analysis(df, images, analysis_data):
    ...
    with tab1:
        ...
        with c1:
            if 'correlation_heatmap' in images:
                st.image(images['correlation_heatmap'], use_container_width=True)
            else:
    ...
    with tab2:
        ...
        with c1:
            if '04_purchase_by_category' in images:
                st.image(images['04_purchase_by_category'], use_container_width=True)
            else:
                df_box = df.copy(); df_box['Category'] = df_box['Category'].astype(str)
                ...
                st.plotly_chart(dark_chart(fig), use_container_width=True)
        with c2:
            age_spend = df.dropna(subset=['Age']).groupby(
                pd.cut(df.dropna(subset=['Age'])['Age'], bins=10)
        ...
        gender_cat = df.groupby(['Category', 'Gender'])['Purchase Amount (USD)'].mean().reset_index()
    ...
    with tab3:
        ...
        if 'chi_square' in analysis_data and not analysis_data['chi_square'].empty:
            st.dataframe(analysis_data['chi_square'], use_container_width=True)
        ...
        st.plotly_chart(dark_chart(fig_ht), use_container_width=True)
    ...
    with tab4:
        ...
        if has_images:
            ...
            with r1c1:
                if '07_seasonal_patterns' in images:
                    st.caption("Seasonal Patterns"); st.image(images['07_seasonal_patterns'], use_container_width=True)
            with r1c2:
                if '09_payment_methods' in images:
                    st.caption("Payment Methods"); st.image(images['09_payment_methods'], use_container_width=True)
            r2c1, r2c2 = st.columns(2)
            with r2c1:
                if '08_subscription_impact' in images:
                    st.caption("Subscription Impact"); st.image(images['08_subscription_impact'], use_container_width=True)
            with r2c2:
                if '10_discount_effectiveness' in images:
                    st.caption("Discount Effectiveness"); st.image(images['10_discount_effectiveness'], use_container_width=True)
        else:
            r1c1, r1c2 = st.columns(2)
            with r1c1:
                if 'Season' in df.columns:
                    seas = df.groupby('Season')['Purchase Amount (USD)'].agg(['mean','count']).reset_index()
            ...
            with r1c2:
                if 'Payment Method' in df.columns:
                    ...
                    st.plotly_chart(dark_chart(fig_p), use_container_width=True)
            r2c1, r2c2 = st.columns(2)
            with r2c1:
                if 'Subscription Status' in df.columns:
                    sub = df.groupby('Subscription Status')['Purchase Amount (USD)'].agg(
                    ...
                    st.plotly_chart(dark_chart(fig_sub), use_container_width=True)
            with r2c2:
                if 'Discount Applied' in df.columns:
                    disc = df.groupby('Discount Applied')['Purchase Amount (USD)'].agg(['mean','count']).reset_index()
        ...
        with c1:
            if '11_customer_loyalty' in images:
                st.image(images['11_customer_loyalty'], use_container_width=True)
        ...
        with c2:
            if '12_geographic_distribution' in images:
                st.image(images['12_geographic_distribution'], use_container_width=True)
            elif 'Location' in df.columns:
                loc = df.groupby('Location')['Purchase Amount (USD)'].agg(
                ...
                fig_l.update_layout(
                    ...
                    height=480,
                    yaxis=dict(tickmode='array', tickvals=loc['Location'].tolist(),
                               ticktext=loc['Location'].tolist(), autorange='reversed')
                )
                st.plotly_chart(dark_chart(fig_l), use_container_width=True)
...
def page_segmentation(df, analysis_data):
    ...
    weekly_pct  = (df[freq_col] == 'Weekly').sum() / len(df) * 100 if freq_col in df.columns else 0
    monthly_pct = (df[freq_col] == 'Monthly').sum() / len(df) * 100 if freq_col in df.columns else 0
    ...
    if 'segments' in analysis_data and not analysis_data['segments'].empty:
        ...
        if 'freq_per_year' in seg_df.columns:
            ...
            with tab2:
                ...
                c1, c2 = st.columns(2)
                with c1:
                    seg_sp = seg_df.groupby('freq_category')['Purchase Amount (USD)'].mean().reset_index()
        ...
        with tab1:
            ...
            if freq_col in df.columns:
                c1, c2 = st.columns(2)
                with c1:
                    freq_counts = df[freq_col].value_counts().reset_index()
                    ...
                    st.plotly_chart(dark_chart(fig), use_container_width=True)
                with c2:
                    freq_spend = df.groupby(freq_col)['Purchase Amount (USD)'].agg(
                    ...
                    st.dataframe(freq_spend, use_container_width=True, hide_index=True)
                    insight("Highest Value Segment",
                            f"<strong>{freq_spend.sort_values('Avg Purchase',ascending=False).iloc[0]['Frequency']}</strong> "
                            f"buyers have the highest average spend at "
                            f"<strong>{fmt_usd(freq_spend['Avg Purchase'].max())}</strong>.", "success")
        ...
        with tab3:
            ...
            with c2:
                if 'Gender' in df.columns:
                    gender_cat = df.groupby(['Gender','Category'])['Purchase Amount (USD)'].mean().reset_index()
        ...
        with tab4:
            ...
            if 'Previous Purchases' in df.columns and freq_col in df.columns:
                ...
                df_rfm = df.copy()
                df_rfm['F_score'] = df_rfm[freq_col].map(freq_map).fillna(1)
                ...
                with c2:
                    ...
                    insight("Champions",
                            f"Your top RFM segment has <strong>{rfm_stats[rfm_stats['Segment']=='Champions']['Count'].sum():,}</strong> "
                            "customers. Prioritise retention with exclusive offers.", "success")
                    insight("At-Risk",
                            f"<strong>{rfm_stats[rfm_stats['Segment']=='At-Risk']['Count'].sum():,}</strong> customers are "
...
def page_insights(df, analysis_data):
    ...
    with tab1:
        ...
        top_cat   = df['Category'].value_counts().index[0]
        top_cat_n = df['Category'].value_counts().iloc[0]
        top_loc   = df['Location'].value_counts().index[0]
        avg_age   = df['Age'].mean()
        avg_pur   = df['Purchase Amount (USD)'].mean()
        sat_pct   = (df['Review Rating'] >= 4).sum() / len(df) * 100
        sub_pct   = (df['Subscription Status'] == 'Yes').sum() / len(df) * 100 if 'Subscription Status' in df.columns else 0
        promo_pct = (df['Promo Code Used'] == 'Yes').sum() / len(df) * 100 if 'Promo Code Used' in df.columns else 0
        top_pay   = df['Payment Method'].value_counts().index[0] if 'Payment Method' in df.columns else 'N/A'
        top_ship  = df['Shipping Type'].value_counts().index[0] if 'Shipping Type' in df.columns else 'N/A'
        disc_avg  = df[df['Discount Applied']=='Yes']['Purchase Amount (USD)'].mean() if 'Discount Applied' in df.columns else 0
        nodisc_avg= df[df['Discount Applied']=='No']['Purchase Amount (USD)'].mean() if 'Discount Applied' in df.columns else 0
        ...
        with c1:
            ...
            insight("Primary Market",
                    ...
                    f"<strong>{df['Location'].value_counts().iloc[0]/len(df)*100:.1f}%</strong> of volume.", "warning")
    ...
    with tab3:
        ...
        if 'Season' in df.columns:
            groups = [g['Purchase Amount (USD)'].dropna().values for _, g in df.groupby('Season') if len(g) > 1]
            if len(groups) >= 2:
        ...
        if 'Category' in df.columns:
            groups = [g['Purchase Amount (USD)'].dropna().values for _, g in df.groupby('Category') if len(g) > 1]
            if len(groups) >= 2:
                F, p = scipy_stats.f_oneway(*groups)
                sig = p < 0.05
        ...
        if 'Discount Applied' in df.columns and 'Review Rating' in df.columns:
            disc_num = (df['Discount Applied'] == 'Yes').astype(int)
        ...
        if test_rows:
            # Summary table first — most prominent
        ...
        # Also show file-based results if they exist
...
def main():
    ...
    if df is None:
        st.error("Unable to load dataset. Please verify the CSV file path.")
        return
